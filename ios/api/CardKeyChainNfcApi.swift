...
class CardKeyChainNfcApi: TonNfcApi {
    ...
    func finishDeleteKeyFromKeyChainAfterInterruption(keyMac: String) {
    ...
    func deleteKeyRecordPromise(sault : Data) -> Promise<Data> {
        return Promise{promise in promise.fulfill(Data(_ : []))}
    ...
    func getDeleteKeyRecordNumOfPackets() {
    ...
    func getDeleteKeyRecordNumOfPacketsPromise() -> Promise<Int> {
    ...
    func getDeleteKeyChunkNumOfPackets() {
    ...
    func getDeleteKeyChunkNumOfPacketsPromise() -> Promise<Int> {
        self.selectTonAppletAndGetSault()
            .then{(sault : Data) -> Promise<Data> in
    ...
    func getIndexAndLenOfKeyInKeyChainPromise(keyHmac: String) -> Promise<Data>{
        let keyHmacBytes = ByteArrayAndHexHelper.hexStrToUInt8Array(hexStr: keyHmac)
        return self.reselectKeyForHmac()
            .then{(response : Data) -> Promise<Data> in
                self.getSaultPromise()
    ...
    func selectTonAppletAndCheckPersonalizedState() -> Promise<Data> {
    ...
    func selectTonAppletAndCheckDeleteState() -> Promise<Data> {
        self.apduRunner.sendTonWalletAppletApdu(apduCommand: TonWalletAppletApduCommands.GET_APP_INFO_APDU)
            .then{(response : Data)  -> Promise<Data> in
                if (response.count != TonWalletAppletConstants.GET_APP_INFO_LE) {
                    throw ResponsesConstants.ERROR_MSG_STATE_RESPONSE_LEN_INCORRECT
