...
import static com.tonnfccard.utils.ByteArrayHelper.bConcat;
...
public class CAPDUTest {
    ...
    public void CAPDUTest2() {
        ...
        for (int le = 0x00; le <= 0xFF; le++) {
            ...
            assertEquals(capdu.toString(), "21320102" + hex((byte) le));
        }
    }
    ...
    public void CAPDUTest3() {
        ...
        for (int len = 0x01; len <= 0xFF; len++) {
            ...
            assertArrayEquals(bConcat(new byte[]{(byte) cla, (byte) ins, (byte) p1, (byte) p2, (byte) dataField.length},  dataField), capdu.getBytes());
            assertEquals(capdu.getCla(), (byte) cla);
            assertEquals(capdu.getIns(), (byte) ins);
            assertEquals(capdu.getP1(), (byte) p1);
            assertEquals(capdu.getP2(), (byte) p2);
            ...
            assertEquals(capdu.getLe(), -1);
            assertEquals(capdu.toString(), "B0A10A0B" + hex((byte) dataField.length) + hex(dataField) );
        }
    }
    ...
    public void CAPDUTest6() {
        ...
        for (int le = 0x00; le <= 0xFF; le++) {
            for (int len = 0x01; len <= 0xFF; len++) {
                ...
                assertArrayEquals(bConcat(new byte[]{(byte) cla, (byte) ins, (byte) p1, (byte) p2, (byte) dataField.length}, dataField, new byte[]{(byte) le}), capdu.getBytes());
                assertEquals(capdu.getCla(), (byte) cla);
                assertEquals(capdu.getIns(), (byte) ins);
                assertEquals(capdu.getP1(), (byte) p1);
                assertEquals(capdu.getP2(), (byte) p2);
                ...
                assertArrayEquals(capdu.getData(), dataField);
                assertEquals(capdu.getLe(), le);
                assertEquals(capdu.toString(), "B0A10A0B" + hex((byte) dataField.length) + hex(dataField) + hex((byte) le));
            }
        }
    }
